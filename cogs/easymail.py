...
class Easymail(commands.Cog):
    ...
    async def send_status(self, ctx: commands.Context, id, description=None):
        ...
        if status['description'] == "Παραδόθηκε":
            await self.remove_id(ctx, id)
    ...
    async def get_last_status(self, id):
        ...
        location = items[2].contents[0].contents[0]

        return 0, {"date": date, "description": description, "location": location.capitalize()}
    ...
    async def store_id(self, ctx: commands.Context, id, description):
        ...
        if id not in self.bot.guild_data[str(ctx.guild.id)]['easymail']:
