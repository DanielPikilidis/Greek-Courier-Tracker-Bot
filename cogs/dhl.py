...
class Dhl(commands.Cog, name="DHL"):
    ...
    async def dhl(self, ctx: commands.Context):
        ...
        embed.add_field(name="?/dhl track <id1> <id2> ...", value="Returns current status for the package(s)", inline=False)
    ...
    async def remove_id(self, ctx: commands.Context, id):
        package = next((i for i in self.bot.guild_data[str(ctx.guild.id)]['dhl'] if i['id'] == id), None)
        if package:
            description = package['description']
            self.bot.guild_data[str(ctx.guild.id)]['dhl'].remove(package)
