...
class Geniki(commands.Cog):
    ...
    async def geniki(self, ctx: commands.Context):
        ...
        embed.add_field(name="?/geniki track <id1> <id2> ...", value="Returns current status for the parcel(s)", inline=False)
    ...
    async def remove_id(self, ctx: commands.Context, id):
        parcel = next((i for i in self.bot.guild_data[str(ctx.guild.id)]['geniki'] if i['id'] == id), None)
        if parcel:
            description = parcel['description']
            self.bot.guild_data[str(ctx.guild.id)]['geniki'].remove(parcel)
