...
class EasyMailTracker(commands.Cog, name="EasyMail"):
    def __init__(self, bot: commands.Bot):
        ...
        self.tracker_url = self.tracker_url + "/track-one/easymail"
    ...
    async def easymail(self, ctx: commands.Context):
        ...
        embed.set_thumbnail(url="https://i.imgur.com/f0PLGma.png")
    ...
    async def update_ids(self):
        ...
        tracking_ids = sqlite3_handler.get_distinct_tracking_ids_for_courier_name("easymail")
        # Checking each id
        for tracking_id in tracking_ids:
            await self.__check_if_changed(tracking_id)

def setup(bot: commands.Bot):
    bot.add_cog(EasyMailTracker(bot))
