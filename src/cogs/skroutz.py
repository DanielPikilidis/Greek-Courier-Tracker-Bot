...
class SkroutzTracker(commands.Cog, name="Skroutz"):
    def __init__(self, bot: commands.Bot):
        ...
        self.tracker_url = self.tracker_url + "/track-one/skroutz/"
    ...
    async def skroutz(self, ctx: commands.Context):
        embed = discord.Embed(
            title="Skroutz help",
            description="All the available subcommands for Skroutz.",
            color=self.colour
        )
        ...
        embed.set_thumbnail(url="https://i.imgur.com/FmhZYBQ.png")
        embed.set_footer(text=f"Skroutz help requested by: {ctx.author.display_name}")
    ...
    async def update_ids(self):
        ...
        tracking_ids = sqlite3_handler.get_distinct_tracking_ids_for_courier_name("skroutz")
        # Checking each id
        for tracking_id in tracking_ids:
            await self.__check_if_changed(tracking_id)

def setup(bot: commands.Bot):
    bot.add_cog(SkroutzTracker(bot))
